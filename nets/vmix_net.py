...
class VMixNet(nn.Module):
    def __init__(self, num_feats, num_classes, num_hidden, num_inLayer=1, num_vsgc=1, bias=False, activation=F.relu, graph_norm=True,
        ...
        self.inLayers.append(GCNLayer(num_feats, num_hidden, bias, activation, dropout=dropout))
        if num_inLayer == 2:
            self.inLayers.append(GCNLayer(num_hidden, num_hidden, bias, None, dropout=dropout))
        elif num_inLayer > 2:
            for _ in range(num_inLayer - 2):
                self.inLayers.append(GCNLayer(num_hidden, num_hidden, bias, activation, dropout=dropout))
            self.inLayers.append(GCNLayer(num_hidden, num_hidden, bias, None, dropout=dropout))
